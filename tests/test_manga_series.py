...
class TestMangaSeries:
    def test_get_al_data(self, manga_series):
        ...
        assert (
            ...
            "flesh is also indelibly marked with The Brand, an unholy symbol that draws the forces of darkness to him "
            "and dooms him as their sacrifice. But Guts won't take his fate lying down; he'll cut a crimson swath of "
            "carnage through the ranks of the damned—and anyone else foolish enough to oppose him! Accompanied by Puck "
            "the Elf, more an annoyance than a companion, Guts relentlessly follows a dark, bloodstained path that "
            "leads only to death...or vengeance."
        )
        assert manga_series.publisher == "Dark Horse"
        assert manga_series.status == "RELEASING"

    def test_get_cv_data(self, manga_series):
    ...
    def test_al_get_author(self, manga_series):
        assert manga_series.al_get_author("Chainsaw Man") == "Tatsuki Fujimoto"

    def test_parse_publisher(self, manga_series, anilist):
        # Case with publisher
    ...
    def test_clean_description(self, manga_series, anilist):
        target_description = (
            "The master spy codenamed <Twilight> has spent his days on undercover missions, all for the dream of a "
            "better world. But one day, he receives a particularly difficult new order from command. For his mission, "
            "he must form a temporary family and start a new life?! A Spy/Action/Comedy about a one-of-a-kind family!"
        )
