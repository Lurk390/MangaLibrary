...
for i in range(1, MangaID_length):
    url = cursor.execute("""
    SELECT URL FROM MangaInfo WHERE MangaID = %s
    """ % i).fetchone()[0]

    # Inserts data from getData(url) into MangaInfo table
    manga_data = getData(url)
    cursor.execute("""
        UPDATE MangaInfo SET Title = ?, Author = ?, Year = ?, Publisher = ?, NumberOfVolumes = ?, Description = ?, Status = ?, CoverImage = ? WHERE MangaID = ?""",
        manga_data["title"],
        manga_data["author"],
        manga_data["year"],
